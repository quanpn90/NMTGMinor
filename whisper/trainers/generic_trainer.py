...
from .trainer_pt_utils import set_rng_state_for_device
...
def compute_steps(state, args, max_steps):
    """
    ...
    for step_kind in ("logging", "eval", "save"):
        ...
        if num_steps is not None:
            ...
            setattr(state, f"{step_kind}_steps", num_steps)


def init_training_references(state, trainer, train_dataloader, max_steps, num_train_epochs, trial):
    """
    Stores the initial training references needed in `self`
    """
    for attr in ("model", "optimizer", "lr_scheduler"):
        setattr(state, attr, getattr(trainer, attr))
    ...
    if trainer.hp_name is not None and trainer._trial is not None:
        ...
        state.trial_name = trainer.hp_name(trainer._trial)
    state.trial_params = None
    if trial is not None:
        from transformers.integrations import hp_params

        assignments = trial.assignments if trainer.hp_search_backend == HPSearchBackend.SIGOPT else trial
        state.trial_params = hp_params(assignments)

    state.max_steps = max_steps
    state.num_train_epochs = num_train_epochs
    state.is_local_process_zero = trainer.is_local_process_zero()
    state.is_world_process_zero = trainer.is_world_process_zero()
...
class GenericTrainer:
    ...
    def __init__(
        ...
        # 2 variables to control the data in training
        # by default they are local variables within huggingface
        self.epoch_dataloader = None
    ...
    def generate_epoch_dataloader(self):
        """
        Added by Quan: this function returns the epoch data loader before every epoch during training
        Returns: None
        """
    ...
    def _inner_training_loop(
        ...
        if self.is_fsdp_xla_v2_enabled:
            self.train_dataloader = tpu_spmd_dataloader(self.train_dataloader)
        ...
        (
            ...
            len_dataloader,
            max_steps,
        ) = self.set_initial_training_values(args, self.train_dataloader, total_train_batch_size)

        num_train_tokens = None
        if self.args.include_tokens_per_second:
            num_train_tokens = self.num_tokens(self.train_dataloader, None if epoch_based else max_steps)
        ...
        compute_steps(self.state, args, max_steps)
        ...
        init_training_references(self.state, self, self.train_dataloader, max_steps, num_train_epochs, trial)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            # self.epoch_dataloader = self.train_dataloader
            self.generate_epoch_dataloader()

            if hasattr(self.epoch_dataloader, "set_epoch"):
                self.epoch_dataloader.set_epoch(epoch)
            ...
            if steps_trained_in_current_epoch > 0:
                self.epoch_dataloader = skip_first_batches(self.epoch_dataloader, steps_trained_in_current_epoch)
                steps_skipped = steps_trained_in_current_epoch
                steps_trained_in_current_epoch = 0
                rng_to_sync = True

            step = -1
            epoch_iterator = iter(self.epoch_dataloader)
