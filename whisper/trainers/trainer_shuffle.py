...
class MemSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def __init__(self, max_epochs=1, eval_data_collator=None, train_dataset_dict=None, *args, **kwargs):
        ...
        # breakpoint()
        #self.train_dataset = kwargs["eval_dataset"] 
    ...
    def train(self, *args, **kwargs):
        """
        Overrides `train()` to update the dataset per epoch dynamically.
        """

        for epoch in range(int(self.max_epochs)):
            self.current_epoch = epoch  # Track epoch

            prev_state = self.state
            prev_control = self.control
            # Call Hugging Face's original training logic
            super().train(*args, **kwargs)
            self.state = prev_state  # Restore previous state
            self.control = prev_control

            # Update step offset for next epoch
    ...
    #         epoch_iterator = self.get_train_dataloader()#train_dataloader
