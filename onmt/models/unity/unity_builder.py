...
from ..speech_recognizer.w2v_bert.w2vbert_conformer_shaw import (
    ConformerShawEncoderBuilder, ConformerShawEncoderConfig, conformer_shaw_archs)

from .unity_model import Wav2Vec2ModelWithAdaptor
...
class UnitYBuilder:
    ...
    def build_speech_encoder_model(self):
        """Build a model."""
        speech_encoder_frontend = self.w2v2_encoder_builder.build_frontend()
        speech_encoder = self.build_speech_encoder()

        return Wav2Vec2ModelWithAdaptor(speech_encoder_frontend, speech_encoder)
...
def create_unity_encoder(
    config: UnitYConfig,
    device: Optional[Device] = None,
    dtype: Optional[DataType] = None,
) :
    """Create a UnitY model encoder (w2vbert + adaptor only).
    ...
    return unity_builder.build_speech_encoder_model()