...
class TransducerLoss(torch.nn.Module):
    ...
    Arguments:
        ...
        data being removed. (default: False)
    """
    def __init__(self, fuse_softmax_backward=True, opt=1, packed_input=False, blank_idx=0):
    ...
    def forward(self, x, label, f_len, y_len, batch_offset=None, max_f_len=None,
                debug_list=None):
        """Forward operation of transducer joint
...
class TransducerLossTorch(torch.nn.Module):
    """Transducer loss
    Detail of this loss function can be found in: Sequence Transduction with Recurrent Neural
    Networks
    ...
    def forward(self, x, label, f_len, y_len):
        """Forward operation of transducer joint
        ...
        """
        blank_idx = self.blank_idx
        # print(x.size(), f_len.size(), label.size(), y_len.size())

        return rnnt_loss(x, label, f_len, y_len, blank_idx, reduction="none", fused_log_softmax=True)
