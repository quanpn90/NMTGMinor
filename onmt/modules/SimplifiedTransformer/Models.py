...
class SimplifiedTransformer(NMTModel):
    ...
    def decode(self, batch):
        ...
        decoder_output = self.decoder(tgt_input, tgt_attbs, context, fake_src)
...
# subclassing the SimplifiedTransformer so that we can inherit the decoding process
# which should be identical
class ParallelSimplifiedTransformer(SimplifiedTransformer):
    """Main model in 'Attention is all you need' """
    ...
    def forward(self, batch):
        ...
        if self.tgt_encoder is not None:
            ...
            fake_tgt = tgt_.new(tgt_context.size(1), tgt_context.size(0)).fill_(onmt.Constants.BOS)

            # generate the target distribution on top of the tgt hiddens
            tgt_dec_output = self.tgt_decoder(tgt, tgt_attbs, tgt_context, fake_tgt, freeze_embedding=True)
        else:
