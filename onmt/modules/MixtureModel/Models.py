...
from onmt.modules.Transformer.Layers import EncoderLayer, DecoderLayer, PositionalEncoding, variational_dropout, PrePostProcessing, Linear
...
class MixtureEncoder(nn.Module):
    ...
    def __init__(self, opt, dicts, positional_encoder):
        ...
        self.n_mixtures = opt.n_mixtures
        ...
        for k in range(self.n_mixtures):
            pplayer_ = PrePostProcessing(self.model_size, 0, sequence='n')
            self.postprocess_layers.append(pplayer_)
    ...
    def build_modules(self):
        ...
        for k in range(self.n_mixtures):
            layer_modules = nn.ModuleList([EncoderLayer(self.n_heads, self.model_size, self.dropout, self.inner_size, self.attn_dropout, self.residual_dropout) for _ in range(self.layers)])
            self.mixtures.append(layer_modules)

        # simple gating function with Softmax
        self.gate = Linear(self.model_size, self.n_mixtures)
    ...
    def forward(self, input, **kwargs):
        ...
        context = emb.transpose(0, 1).contiguous() # T x B x H
        gate_input = context

        mixture_outputs = list()
        for k, mixture in enumerate(self.mixtures):

            context_ = context
            for i, layer in enumerate(mixture):
                
                if len(mixture) - i <= onmt.Constants.checkpointing and self.training:        
                    context_ = checkpoint(custom_layer(layer), context_, mask_src)

                else:
                    context_ = layer(context_, mask_src)      # batch_size x len_src x d_model
            ...
            context_ = self.postprocess_layers[k](context_)

            context_ = context_.unsqueeze(2) # T x B x 1 x H

            mixture_outputs.append(context_)

        contexts = torch.cat(mixture_outputs, dim=2) # T x B x K x H

        prob_gates = F.softmax(self.gate(gate_input).float(), dim=-1).type_as(contexts) # T x B x K

        prob_gates = prob_gates.unsqueeze(2)
        context = torch.matmul(prob_gates, contexts).squeeze(2)
...
class MixtureDecoder(nn.Module):
    ...
    def build_modules(self):
        ...
        self.mixtures = nn.ModuleList()
        for k in range(self.n_mixtures):
            layer_modules = nn.ModuleList([DecoderLayer(self.n_heads, self.model_size, self.dropout, self.inner_size, self.attn_dropout, self.residual_dropout) for _ in range(self.layers)])
            self.mixtures.append(layer_modules)
        ...
        for k, mixture in enumerate(self.mixtures):
            ...
            for i, layer in enumerate(mixture):
                ...
                if len(mixture) - i <= onmt.Constants.checkpointing and self.training:           
                    ...
                    output_, coverage = checkpoint(custom_layer(layer), output_, context, mask_tgt, mask_src) 
                                                                                  # batch_size x len_src x d_model
                    
                else:
                    output_, coverage = layer(output_, context, mask_tgt, mask_src) # batch_size x len_src x d_model
            
            # From Google T2T
            # if normalization is done in layer_preprocess, then it should also be done
            # on the output, since the output can grow very large, being the sum of
            # a whole stack of unnormalized layer outputs.    
            output_ = self.postprocess_layers[k](output_)
            mixture_outputs.append(output_.unsqueeze(2))
        
        outputs = torch.cat(mixture_outputs, dim=2) # T x B x K x H

        prob_gates = F.softmax(self.gate(gate_input).float(), dim=-1).type_as(outputs) # T x B x K

        prob_gates = prob_gates.unsqueeze(2)
        output = torch.matmul(prob_gates, outputs).squeeze(2)
