...
SKIP_TEST = None
try:
    from optimized import softmax_xentropy as label_smoothing
...
def label_smoothing_raw(x, target, padding_idx, smoothing):
    loss = F.cross_entropy(x.float(), target, weight=None,
                           ignore_index=padding_idx, reduction='none',
                           label_smoothing=smoothing).float()
    ...
    def _test_label_smoothing_function(self, dtype):
        ...
        for i in range(iters):
            logits, labels, half_to_float = self.gen_test_inputs(
                N, T, H, smoothing, padding_idx, dtype=dtype)
            print(dtype, half_to_float)
            ...
            logits.grad = None
            half_to_float = False
            losses = loss_func(logits.float(), labels, smoothing, padding_idx, half_to_float)
            loss = losses.sum()
            loss.backward()
