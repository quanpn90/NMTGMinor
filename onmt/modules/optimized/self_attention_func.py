...
class SelfAttnBiasFunc(torch.autograd.Function):
    ...
    def forward(ctx, use_time_mask, is_training, heads, inputs, attn_bias,
                ...
                incremental, incremental_cache,
                low_precision, return_coverage, recompute):
        # the function does not support recompute
        recompute = False
        ...
        if low_precision and self_multihead_attn_blaslt is not None and not incremental and len_q <= 2048 \
            ...
            if mask is not None:
                if use_time_mask:
                    ...
                    mask = mask.unsqueeze(1).unsqueeze(2).bool()
                    mask = mask.repeat(1, heads, 1, 1).view(bsz * heads, 1, -1)

                    # IMPORTANT: fill the attn_bias with -inf before calling the forward
                    attn_bias.masked_fill_(mask, float('-inf'))
            else:
                if use_time_mask:
                    mask = inputs.new(len_q, len_q).zero_().bool()
                else:
                    mask = inputs.new(1, 1, 1, 1).zero_().bool()  # works
            ...
            dropout_mask, \
            matmul2_results, \
            outputs = cuda_module.forward_bias(use_time_mask, is_training, heads,
                                                       inputs.contiguous(), input_weights, output_weights,
                                                       input_biases, output_biases,
        ...
        # output:           [seqs*heads, seql_q, seql_k]
        # GEMM: Per batch: ( seql_q x head_dim ) x ( head_dim x seql_k ) = ( seql_q x seql_k )
        # print("USING BADDBMM 2 inplace from attn bias ...")
        ...
        #     matmul1_results.mul_(scale_t[0])
        #
        # matmul1_results = matmul1_results + attn_bias
        ...
        if mask is not None:
            ...
            if use_time_mask:
                ...
                mask = mask.to(torch.bool)
                matmul1_results = matmul1_results.masked_fill_(mask, float('-inf'))
            # no need to apply mask to matmul1 results again
            ...
            if input_weights.requires_grad:
                ...
                if ctx.recompute:
                    ...
                    input_grads, \
                        ...
                        output_bias_grads,\
                        attn_bias_grads = \
                        cuda_module.backward_bias(ctx.use_time_mask, heads_t[0],
                                              output_grads.contiguous(), matmul2_results,
                                              dropout_results, attn_scores,
                                              input_lin_results, inputs, input_weights,
                                              output_weights, dropout_mask, dropout_prob_t[0])
...
if __name__ == "__main__":
    ...
    class TestAttention(torch.nn.Module):
        ...
        def forward(self, input_weights, output_weights, input, attn_bias, input_biases, output_biases, mask,
                    use_time_mask=False):
            is_training = True
            dropout = 0.0
    ...
    input_states = torch.randn(*(len_q, bsz, opt.model_size)).double().cuda() * 0.01
    ...
    mask = input_states.new(*(bsz, len_r)).bernoulli_(p=0.25).bool().zero_()

    attn_bias = torch.randn(*(bsz * opt.n_heads, len_q, len_r)).double().cuda()* 1

    # input_states.new(*(bsz * opt.n_heads, len_q, len-r))

    print("gradchecking start.")

    use_time_mask = False

    torch.autograd.gradcheck(net, (in_proj_weight, out_proj_weight, input_states, attn_bias,
                                   in_proj_bias, out_proj_bias,
                                   mask, use_time_mask), atol=1e-03, rtol=0.001)
