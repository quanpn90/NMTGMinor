...
class SelfMultiheadAttn(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=None,
        ...
        if not query.is_cuda:
            return self.forward_autograd(attn_mask is not None, is_training, self.num_heads, query,
                                         input_weights, self.out_proj_weight,
                                         input_bias, self.out_proj_bias,
                                         mask, self.dropout,
                                         incremental, incremental_cache)

        if self.optimized == 1 and self.training and len_key <= 1024:
            if mask is not None:
                mask = mask.byte()
    ...
    def forward_autograd(self, use_time_mask, training, heads, inputs, input_weights, output_weights,
        ...
        qk_scores = torch.matmul(queries.transpose(0, 1), keys.transpose(0, 1).transpose(1, 2))
        qk_scores.mul_(scale_t[0])
        ...
        attn_scores = F.softmax(qk_scores.double(), dim=-1).type_as(qk_scores)

        # nan_mask = torch.isnan(attn_scores)
        # print(nan_mask.float().sum())
