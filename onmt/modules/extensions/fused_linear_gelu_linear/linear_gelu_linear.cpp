...
template <typename T>
int linear_gelu_linear_forward_cuda_direct(T *input, T *weight1, T *bias1,
                                                     T *weight2, T *bias2,
                                                     int in_features, int hidden_features, int batch_size, int out_features,
                                                     T *output1, T *output2, void *lt_workspace) ;

template <typename T>
int linear_gelu_linear_backward_cuda_recompute(T *input, T *weight1, T *weight2, T *d_output1, T *d_output2, int in_features, int batch_size, int hidden_features, int out_features, T *d_weight1, T *d_weight2, T *d_bias1, T *d_bias2, T *d_input, void *lt_workspace);
...
// this function computes linear-gelu-linear without storing gelu_in and returning output1
std::vector<at::Tensor> linear_gelu_linear_forward_direct(at::Tensor input, at::Tensor weight1, at::Tensor bias1, at::Tensor weight2, at::Tensor bias2) {
  ...
  // create output/workspace tensor
  // we still need the output1 which is the input for the 2nd linear. but don't return it.
  auto output1 = at::empty({batch_size, hidden_features}, input.type());
  auto output2 = at::empty({batch_size, out_features}, input.type());
  //auto reserved_space = at::empty({reserved_size}, inputs[0].type());
  // allocate fixed 4MB workspace for cublaslt for now, and this gets at least 4 MB
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.type(), "linear_gelu_linear_forward_direct", [&] {
    scalar_t* w1_ptr = weight1.data_ptr<scalar_t>();
    scalar_t* b1_ptr = bias1.data_ptr<scalar_t>();
    scalar_t* w2_ptr = weight2.data_ptr<scalar_t>();
    scalar_t* b2_ptr = bias2.data_ptr<scalar_t>();
...
std::vector<at::Tensor> linear_gelu_linear_backward_recompute(at::Tensor input, at::Tensor weight1, at::Tensor weight2, at::Tensor d_output2) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.type(), "linear_bias_backward_recompute", [&] {
    //scalar_t* w_ptr = weight.data_ptr<scalar_t>();
    //scalar_t* d_b_ptr = d_bias.data_ptr<scalar_t>();
    auto result = linear_gelu_linear_backward_cuda<scalar_t>(
        input.data_ptr<scalar_t>(),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("linear_gelu_linear_forward_direct", &linear_gelu_linear_forward, "linear gelu linear forward");
  m.def("linear_gelu_linear_backward_recompute", &linear_gelu_linear_backward, "linear gelu linear backward");
}
