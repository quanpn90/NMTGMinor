...
std::vector<torch::Tensor> fwd_compact_cuda(
							   ...
							   bool                 is_training,
                               int                  heads,
                               torch::Tensor const& input_lin_results,  // need to return the gradients for this
                               torch::Tensor const& pad_mask,
                               float                dropout_prob,
                               torch::Tensor lt_workspace)

{
  int   embed_dim      = input_lin_results.size(2) / 3;
  int   sequences      = input_lin_results.size(1);
  int   q_seq_len      = input_lin_results.size(0);
  int   k_seq_len      = q_seq_len;
  int   batches        = sequences * q_seq_len;
  int   head_dim       = embed_dim / heads;
  int   output_lin_dim = input_lin_results.size(2);
  int   attn_batches   = heads * sequences;
  int   lead_dim       = attn_batches * 3 * head_dim;
  int   batch_stride   = 3 * head_dim;
  int   dropout_elems  = attn_batches * q_seq_len * k_seq_len;
  ...
  // 3 Intermediate Results + Output (Note: dropout intermediates are generated by ATen library code)
  auto act_options  = input_lin_results.options().requires_grad(false);
  auto mask_options = act_options.dtype(torch::kUInt8);
...
//   auto q_lin_grads_ptr = static_cast<half*>(input_lin_output_grads.data_ptr());
  ...
  if (cublas_status != CUBLAS_STATUS_SUCCESS) {
      printf("Strided Batched GEMM backward MatMul2 Dgrad1 failed with %d\n", cublas_status);
      exit(0);
  }
