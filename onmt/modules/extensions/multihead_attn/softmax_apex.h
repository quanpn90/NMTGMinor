...
template<typename input_t, typename output_t, typename acc_t, bool is_log_softmax>
using softmax_warp_backward_norecompute_func = void(*)(output_t *gradInput, const input_t *grad, const input_t *output, int batch_size, int stride, int pad_batch_stride, int element_count);
...
template<typename input_t, typename output_t, typename acc_t, bool is_log_softmax>
bool dispatch_softmax_backward_norecompute(output_t *grad_input, const input_t *grad, const input_t *output, int softmax_elements, int softmax_elements_stride, int pad_batch_stride, int batch_count, cudaStream_t streamid)
{
    ...
    if (softmax_elements == 0) {
        ...
        // launch
        kernel<<<blocks, threads, 0, streamid>>>(grad_input, grad, output, batch_count, softmax_elements_stride, pad_batch_stride, softmax_elements);
        return true;
    }
    return false;
}
