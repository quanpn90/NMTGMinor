...
__global__ void additive_masked_softmax_dropout_warp_forward_vec4(output_t *dst, uint8_t *dropout_mask, const input_t *src, const input_t *pad_mask, int batch_size, int stride, int element_count, int pad_batch_stride, at::PhiloxCudaState philox_args, float p)
{
    ...
    for (int i = 0;i < WARP_BATCH;++i) {
    	...
    	//masking_value is a large negative value
                elements_input[i][it + element] = -65504;
            }
...
// ELEMENTS_PER_LDG_STG has to be 1.
template <typename input_t, typename output_t, typename acc_t>
using additive_masked_softmax_dropout_forward_func = void(*)(output_t *dst,
...
template <typename input_t, typename output_t, typename acc_t>
bool warp_additive_masked_softmax_dropout_kernel(int element_count,
                                                 int log2_elements,
                                                 int &warp_size, int &batches_per_warp,
                                                 additive_masked_softmax_dropout_forward_func<input_t, output_t,
                                                 acc_t> &kernel)
{
    // determine size of a warp
    const int next_power_of_two = 1 << log2_elements;
...
int WARP_SIZE = 32, int ELEMENTS_PER_LDG_STG=1>
__global__ void additive_masked_softmax_warp_forward(input_t *dst, const output_t *src,
                                                     const input_t *pad_mask, int batch_size,
...
// ELEMENTS_PER_LDG_STG has to be 1.
template <typename input_t, typename output_t>
using additive_masked_softmax_forward_func = void(*)(input_t *dst,
const output_t *src, const half *pad_mask,
int batch_size, int stride,
int element_count, int pad_batch_stride);
 
template <typename input_t, typename output_t, typename acc_t>
bool warp_additive_masked_softmax_kernel(int log2_elements, int &warp_size,
...
int pad_batch_stride)
{
    if (softmax_elements == 0) {
        ...
        additive_masked_softmax_forward_func<input_t, output_t> kernel;
        int warp_size, batches_per_warp;
        if (!warp_additive_masked_softmax_kernel<input_t, output_t, acc_t>(log2_elements,
                                                                            warp_size,
                                                                            batches_per_warp, kernel))
        {
            return false;
        }
...
template<typename input_t, typename output_t, typename acc_t>
bool dispatch_additive_masked_softmax_stream(output_t *dst,
const input_t *src,
const input_t *pad_mask,
int softmax_elements,
int softmax_elements_stride,
...
cudaStream_t streamid)
{
    if (softmax_elements == 0) {
        ...
        dim3 threads(warp_size, warps_per_block, 1);
        // launch
        kernel<<<blocks, threads, 0, streamid>>>(dst, src,
                                                pad_mask, batch_count,
...
int WARP_ITERATIONS, int WARP_SIZE = 32,
int ELEMENTS_PER_LDG_STG=1>
__global__ void masked_softmax_warp_forward(input_t *dst, const output_t *src,
                                            const uint8_t *pad_mask, int batch_size,
...
__global__ void time_masked_softmax_warp_forward(input_t *dst,
const output_t *src,
...
// ELEMENTS_PER_LDG_STG has to be 1.
template <typename input_t, typename output_t>
using time_masked_softmax_forward_func = void(*)(input_t *dst,
const output_t *src, const uint8_t *pad_mask, int batch_size, int stride, int element_count, int mod_seq_len);
 
template <typename input_t, typename output_t, typename acc_t>
bool warp_time_masked_softmax_kernel(int log2_elements,
int &warp_size, int &batches_per_warp, time_masked_softmax_forward_func<input_t, output_t> &kernel) {
    // determine size of a warp
    const int next_power_of_two = 1 << log2_elements;
    warp_size = (next_power_of_two < 32) ? next_power_of_two : 32;
...
template<typename input_t, typename output_t, typename acc_t>
bool dispatch_time_masked_softmax(
                                output_t *dst,
                                const input_t *src,
                                const uint8_t *pad_mask,
                                int softmax_elements,
                                ...
                                int mod_seq_len)
{
    if (softmax_elements == 0) {
        ...
        int warp_size, batches_per_warp;
        if (!warp_time_masked_softmax_kernel<input_t, output_t, acc_t>(log2_elements,
                                                                       warp_size,
                                                                       batches_per_warp,
                                                                       kernel)) {
            return false;
        ...
        // launch
        kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(dst,
                                                                         src,
                                                                         pad_mask,
                                                                         batch_count,
...
//softmax backward data function is taken from native pytorch,
//elementwise mul is fused in the epolog, as well as masking and scaling for fusing dropout
...
template<typename input_t, typename output_t, typename acc_t, bool is_log_softmax>
void dispatch_masked_scale_softmax_backward_masked_out_stream(output_t *grad_input,
                                                                const input_t *grad,
