...
class VariationalDecoder(TransformerDecoder):
    ...
    def build_modules(self, encoder_to_share=None):
        ...
        if encoder_to_share is None:
            self.layer_modules = nn.ModuleList([DecoderLayer(self.n_heads, self.model_size, self.dropout,
                                                             self.inner_size, self.attn_dropout, self.residual_dropout,
                                                             ignore_source=self.ignore_source) for _ in
                                                range(self.layers)])
        else:
...
class VariationalTransformer(NMTModel):
    ...
    def decode_gold(self, src, tgt):
        ...
        z = p_z.mean.type_as(encoder_meaning)

        gold_scores = encoder_meaning.new(batch_size).zero_()
        gold_words = 0

        output, coverage = self.decoder(tgt_input, encoder_context, src, z)
