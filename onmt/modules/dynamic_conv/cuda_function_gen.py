...
#include "dynamic_conv_cuda.cuh"

std::vector<at::Tensor> dynamic_conv_cuda_forward(at::Tensor input, at::Tensor weight, int padding_l) {
...
"""
    ...
    main_block = """
            if (padding_l == {pad}) {{
                AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.scalar_type(), "dynamic_conv_forward", ([&] {{
                    dynamic_conv_forward_kernel<{k}, {b_size}, {pad}, scalar_t>
...
"""
    ...
    with open("dynamic_conv_cuda_forward.cu", 'w') as forward:
        forward.write(head)
...
std::vector<at::Tensor> dynamic_conv_cuda_backward(at::Tensor gradOutput, int padding_l, at::Tensor input, at::Tensor weight) {
...
"""
    ...
    main_block = """
                if (padding_l == {p}) {{
                    AT_DISPATCH_FLOATING_TYPES_AND_HALF(gradOutput.scalar_type(), "dynamic_conv_backward", ([&] {{
                        dynamic_conv_backward_kernel<{k}, {b_size}, {p}, scalar_t>
                        <<<blocks, {b_size}, 0, stream>>>(
...
"""
    ...
    with open("dynamic_conv_cuda_backward.cu", 'w') as backward:
        backward.write(head)
        for seq in seqs:
