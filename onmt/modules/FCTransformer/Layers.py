...
class HierarchicalMultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, mask=None, query_mask=None, value_mask=None):
        ...
        proj_key   = self.fc_key(key, mask=key_mask).view(b, -1, self.h * self.d_head)            # batch_size x (n_layer x len_key) x h*d_head
        proj_value = self.fc_value(value, mask=value_mask).view(b, -1, self.h * self.d_head)        # batch_size x (n_layer x len_key) x h*d_head
        ...
        out = out.transpose(1, 3).unsqueeze(2).contiguous().view(-1, self.h * self.d_head).view(b, -1, self.h*self.d_head) # batch_size x len_query x h*d_head
