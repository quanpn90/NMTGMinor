...
class StackedLSTM(nn.Module):
    ...
    def state0(self, input, volatile=False):
            """
            ...
            h_size = (self.num_layers, batch_size, self.rnn_size)
            h_0 = Variable(input.data.new(*h_size).zero_(),
                    requires_grad=False)
            c_0 = Variable(input.data.new(*h_size).zero_(),
                    requires_grad=False)
            return (h_0, c_0)