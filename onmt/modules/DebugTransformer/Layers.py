...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, h, d_model, p, d_ff, attn_p=0.1, residual_p=0.1, version=1.0):
        ...
        self.dropout = residual_p
        self.relu_dropout = p
        self.fc1 = Linear(d_model, d_ff)
        self.fc2 = Linear(d_ff, d_model)
        self.layer_norms = nn.ModuleList([nn.LayerNorm(d_model) for i in range(2)])
    ...
    def forward(self, input, attn_mask, pad_mask=None):
        ...
        x, _ = self.multihead(x, x, x, attn_mask)
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, h, d_model, p, d_ff, attn_p=0.1, residual_p=0.1, version=1.0):
        ...
        self.fc1 = Linear(d_model, d_ff)
        self.fc2 = Linear(d_ff, d_model)
        self.layer_norms = nn.ModuleList([nn.LayerNorm(d_model) for i in range(3)])
    ...
    def forward(self, input, context, mask_tgt, mask_src, pad_mask_tgt=None, pad_mask_src=None, residual_dropout=0.0):
        ...
        residual = x
        x = self.layer_norms[0](x)
        x, coverage = self.multihead_tgt(x, x, x, mask_tgt)
        
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x

        x = self.layer_norms[1](x)
        x, coverage = self.multihead_src(x, context, context, mask_src)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
