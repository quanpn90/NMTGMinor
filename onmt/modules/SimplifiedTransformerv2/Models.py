...
class SimplifiedTransformerEncoder(TransformerEncoder):
    ...
    def build_modules(self, shared_encoder=None):
        ...
        if shared_encoder is not None:
            ...
            self.final_projector = shared_encoder.projector
            ...
            self.final_projector = nn.Linear(self.model_size, self.model_size)
    ...
    def forward(self, input, freeze_embedding=False, return_stack=False, additional_sequence=None, **kwargs):
        ...
        mask = input.eq(onmt.Constants.PAD).transpose(0, 1).unsqueeze(-1).unsqueeze(-1) # T x B x 1

        if self.pooling == 'mean':
            outputs = [ mean_with_mask(output_, mask) for output_ in output_split ] # B x h x H
        elif self.pooling == 'max':
            outputs = [ max_with_mask(output, mask) for output_ in output_split ]
        else:
            raise NotImplementedError

        output = torch.stack(outputs) # h x B x H

        # linear transform
        # self.encoder_heads, model_size
        output = torch.nn.functional.elu(self.final_projector(output))
