...
class NeuralPrior(nn.Module):
    ...
    def __init__(self, opt, dicts, positional_encoder):
        ...
        self.mean_predictor = Linear(opt.model_size, self.n_layers * opt.model_size)
        self.var_predictor = Linear(opt.model_size, self.n_layers * opt.model_size)
# 
    def forward(self, context_mean, input, **kwargs):
        ...
        mean = mean.view(mean.size(0), self.n_layers, -1)
        var = var.view(var.size(0), self.n_layers, -1)

        # p_z = torch.distributions.bernoulli.Bernoulli(layer_probs.float())

        # p_zs = torch.split(layer_probs, 1, dim=1)
        means_ = torch.split(mean, 1, dim=1)
        vars_  = torch.split(var,  1, dim=1)

        p_z = list()

        for (mean_, var_) in zip(means_, vars_):
            p_z_i =  torch.distributions.normal.Normal(mean_.squeeze(1).float(), var_.squeeze(1).float())
            p_z.append(p_z_i)
