...
class EncdecAttnFunc(torch.autograd.Function):
    ...
    def forward(ctx, use_time_mask, is_training, heads, inputs_q, inputs_kv,
        ...
        # print(q.size())
        queries = q.view(inputs_q.size(0), inputs_q.size(1) * heads, head_dim)
        ...
        if incremental and ('c_k' in incremental_cache and 'c_v' in incremental_cache):
            ...
            kv = kv.view(inputs_kv.size(0), inputs_kv.size(1) * heads, 2, head_dim)
            keys = kv[:, :, 0, :]
            values = kv[:, :, 1, :]
            if incremental:
        ...
        ctx.save_for_backward(heads_t,
                              ...
                              output_weights, output_biases, r_o, s_o,
                              dropout_mask,
                              dropout_prob_t)
    ...
    def backward(ctx, output_grads, softmax_grads):
        ...
        heads_t, scale_t, matmul2_results, dropout_results, softmax_results \
            , q, q_mm, q_r, kv, kv_mm, kv_r, o_mm, o_r \
            , inputs_q, inputs_kv \
            , input_weights_q, input_biases_q, r_q, s_q \
            , input_weights_kv, input_biases_kv, r_kv, s_kv \
            , output_weights, output_biases, r_o, s_o \
        ...
        return None, None, None \
            ...
            , r_q_grads, s_q_grads, r_kv_grads, s_kv_grads, r_o_grads, s_o_grads \
...
class BEEncdecMultiheadAttn(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None, incremental=False, incremental_cache=None,
        ...
        with torch.no_grad():
            ...
            # indices = indices.repeat(self.num_heads)
        ...
        outputs, coverage = self.attn_func(time_masking, is_training, self.num_heads, query, key,
