...
#~ 
    ...
    def train_epoch(self, epoch, resume=False, batchOrder=None, iteration=0):
        ...
        for i in range(iteration, nSamples):
            ...
            try:
                ...
                # use karg grow to control flow ?
                outputs = self.model(batch, grow=True)
    ...
    def run(self, save_file=None):
        ...
        n_new_layers = opt.grow_layer
        
        print('Loading pre-trained model and optim from checkpoint at %s' % save_file)
        
        if not opt.continue_training:
            self.model.load_state_dict(checkpoint['model'])
        
        
        print('Marking the models to be pretrained')
        
        model.mark_pretrained()
        
        print('Growing %d new layers ...' % opt.grow_layer)
        self.opt.layers += self.opt.grow_layer
        
        model.encoder.add_layers(opt.grow_layer)
        model.decoder.add_layers(opt.grow_layer)
        ...
        # don't initialize the model parameters
