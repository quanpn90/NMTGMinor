...
parser = argparse.ArgumentParser(description='sample_nets.py')
...
parser.add_argument('-config_output', default='config.txt',
                    help="""Path to output the predictions (each line will
                    be the decoded sequence""")
...
def main():
    ...
    config_outF = open(opt.config_output, 'w')
    ...
    translator = onmt.NetworkSampler(opt)
    ...
    for line in addone(inFile):
        ...
        predBatch, predScore, predLength, goldScore, numGoldWords, setup  = translator.translate(srcBatch,
                                                                                    tgtBatch)
        if opt.normalize:
            predBatch_ = []
            predScore_ = []
    ...
    config_outF.write('EMB ')
    for layer in setup:
        config_outF.write(str(layer) + ' ')
    config_outF.write('OUT')
