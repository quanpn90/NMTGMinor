...
if __name__ == '__main__':
    ...
    class TestMLP(unittest.TestCase):
        ...
        def test_with_bias_bfloat16(self):
            torch.set_float32_matmul_precision('high')

            for use_activation in ['relu']:
                mlp = MLP(mlp_sizes, activation=use_activation).cuda().to(torch.bfloat16)

                ref_mlp = deepcopy(mlp)

                test_input = torch.empty(batch_size, mlp_sizes[0], device="cuda", dtype=torch.bfloat16).uniform_(-1., 1.).requires_grad_()
                ref_input = test_input.clone().detach().requires_grad_()
                mlp_out, dropout_mask = mlp(test_input)
                ref_out = ref_mlp(ref_input, dropout_mask, ref=True)
                np.testing.assert_allclose(
                    mlp_out.float().detach().cpu().numpy(),
                    ref_out.float().detach().cpu().numpy(),
                    atol=1e-3, rtol=1e-2)
                ...
                ref_out.mean().mul(10.).backward()
                np.testing.assert_allclose(
                    test_input.grad.float().detach().cpu().numpy(),
                    ref_input.grad.float().detach().cpu().numpy(),
                    atol=1e-3, rtol=1e-2)

                for l in range(mlp.num_layers):
                    np.testing.assert_allclose(
                        mlp.weights[l].grad.float().detach().cpu().numpy(),
                        ref_mlp.weights[l].grad.float().detach().cpu().numpy(),
                        atol=1e-3, rtol=1e-2)
                    np.testing.assert_allclose(
                        mlp.biases[l].grad.float().detach().cpu().numpy(),
                        ref_mlp.biases[l].grad.float().detach().cpu().numpy(),
                        atol=1e-3, rtol=1e-2)
        ...
        #         torch.cuda.profiler.stop()
        #
        # def test_performance_bfloat(self):
        #
        #     for dropout in [0.0, 0.5]:
        #         mlp = MLP(mlp_sizes).cuda().to(torch.bfloat16)
        #
        #         ref_mlp_fast = MLP(mlp_sizes, recompute=False, dropout=dropout).cuda().to(torch.bfloat16)
        #
        #         mlp_layers = []
        #         for i in range(mlp.num_layers):
        #             linear = nn.Linear(mlp_sizes[i], mlp_sizes[i + 1])
        ...
        #         print("Compiling ref mlp ...")
        #         ref_mlp = torch.compile(ref_mlp)
        #         ref_mlp = ref_mlp.to(torch.bfloat16)
        #
        #         test_input = torch.empty(
        #             seq_len, batch_size, mlp_sizes[0], device="cuda", dtype=torch.bfloat16).fill_(10.).requires_grad_()
        #         ref_input = torch.empty(
        #             seq_len, batch_size // 2, mlp_sizes[0], device="cuda", dtype=torch.bfloat16).fill_(10.).requires_grad_()
        #
        #         # Warm up GPU
        #         for _ in range(100):
        #             ref_out = ref_mlp(ref_input)
        ...
        #         torch.cuda.synchronize()
        #         stop_time = time()
        #         print(F"\nPytorch MLP time bf16 {(stop_time - start_time) * 1000. / num_iters:.4f} ms")
        #
        #         torch.cuda.synchronize()
        #         start_time = time()
        #         for _ in range(num_iters):
        ...
        #         torch.cuda.synchronize()
        #         stop_time = time()
        #         print(F"C++ MLP recompute time bf16 {(stop_time - start_time) * 1000. / num_iters:.4f} ms")
        #         torch.cuda.profiler.stop()
        #
        #         torch.cuda.synchronize()
        #         start_time = time()
        ...
        #         torch.cuda.synchronize()
        #         stop_time = time()
        #         print(F"C++ MLP time bf16 {(stop_time - start_time) * 1000. / num_iters:.4f} ms")
        #         torch.cuda.profiler.stop()
