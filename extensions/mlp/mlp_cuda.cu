...
// Activation support fuesed ReLU. Safe to call in-place.
template <typename T>
__global__ void biasAddDropoutRelu_fprop(T *X, uint8_t* mask, T *b, uint batch_size, uint features, float p,
...
//          printf("RELU\n");
          if (p == 0) {
            ...
            biasAddDropoutRelu_fprop<<<num_SMs*num_blocks, BIAS_RELU_FW_NTHREADS, 0,
                                 stream>>>(output, mask, bias, batch_size, input_size, p, rng_engine_inputs);
          }
